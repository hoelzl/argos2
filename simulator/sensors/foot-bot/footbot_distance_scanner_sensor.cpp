...
#include "footbot_distance_scanner_sensor.h"
...
namespace argos {
   ...
   CFootBotDistanceScannerSensor::CFootBotDistanceScannerSensor() :
   ...
   void CFootBotDistanceScannerSensor::Init(TConfigurationNode& t_tree) {
      try {
         ...
         THROW_ARGOSEXCEPTION_NESTED("Initialization error in foot-bot distance scanner sensor.", ex);
      }
   }
   ...
   void CFootBotDistanceScannerSensor::Update() {
      /* Clear the maps */
      ...
      if(m_pcDistScanEntity->GetMode() != CDistanceScannerEquippedEntity::MODE_OFF) {
         /* Update the readings wrt to device mode */
         if(m_pcDistScanEntity->GetMode() == CDistanceScannerEquippedEntity::MODE_POSITION_CONTROL) {
   ...
   void CFootBotDistanceScannerSensor::Reset() {
      /* Clear the maps */
	   m_tReadingsMap.clear();
	   m_tShortReadingsMap.clear();
	   m_tLongReadingsMap.clear();
   ...
   void CFootBotDistanceScannerSensor::UpdateNotRotating() {
      /* NOTE:
         - in the 3D space axis follow the right hand rule,
         which makes X the forward vector and Y the *left* vector
         - on the robot axis follow the left hand rule,
         which makes X the forward vector and Y the *right* vector
      */
      /* Short range [0] */
...
#define ADD_READINGS(RAYS,MAP,MINDIST)              \
   ...
   void CFootBotDistanceScannerSensor::UpdateRotating() {
      /* NOTE:
         - in the 3D space axis follow the right hand rule,
         which makes X the forward vector and Y the *left* vector
         - on the robot axis follow the left hand rule,
   ...
   Real CFootBotDistanceScannerSensor::CalculateReadingForRay(const CRay& c_ray,
                                                              Real f_min_distance) {
      ...
      if(m_cSpace.GetClosestEmbodiedEntityIntersectedByRay(sIntersection, c_ray)) {
         if(m_bShowRays) GetEntity().GetControllableEntity().AddIntersectionPoint(c_ray, sIntersection.TOnRay);
         ...
         if(fDistance > f_min_distance) {
            ...
            if(m_bShowRays) GetEntity().GetControllableEntity().AddCheckedRay(true, c_ray);
...
#define CALCULATE_LONG_RANGE_RAY(ANGLE,INDEX)               \
   ...
   void CFootBotDistanceScannerSensor::CalculateRaysNotRotating() {
   ...
   void CFootBotDistanceScannerSensor::CalculateRaysRotating() {
      /* We make the assumption that the foot-bot is rotated only around Z */
      /* Get the foot-bot orientation */
      CRadians cTmp1, cTmp2, cOrientationZ;
      m_pcEmbodiedEntity->GetOrientation().ToEulerAngles(cOrientationZ, cTmp1, cTmp2);
   ...
   REGISTER_SENSOR(CFootBotDistanceScannerSensor,
