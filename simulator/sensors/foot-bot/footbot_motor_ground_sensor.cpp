...
/**
 * @file <argos2/simulator/sensors/foot-bot/footbot_motor_ground_sensor.cpp>
 *
 * @author Giovanni Pini - <gpini@ulb.ac.be>
 */

#include "footbot_motor_ground_sensor.h"
...
namespace argos {
   ...
   CFootBotMotorGroundSensor::CFootBotMotorGroundSensor() :
   ...
   void CFootBotMotorGroundSensor::Reset() {
      for(UInt32 i = 0; i < CCI_FootBotMotorGroundSensor::NUM_READINGS; ++i) {
   ...
   void CFootBotMotorGroundSensor::AddNoise(UInt32 un_sensor_index) {
   ...
   REGISTER_SENSOR(CFootBotMotorGroundSensor,
                   "footbot_motor_ground", "rot_z_only",
                   "The foot-bot motor ground sensor (optimized for 2D)",
                   "Giovanni Pini [gpini@ulb.ac.be]",
                   "This sensor accesses the foot-bot motor ground sensor. For a complete\n"
