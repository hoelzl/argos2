...
namespace argos {
...
#endif
   {
      ...
      InitializeArena();
      /* Create visitor */
      m_pcVisitor = new CQTOpenGLVisitor(*this,
                                         m_cUserFunctions);
   }
   ...
   void CQTOpenGLWidget::DrawScene() {
      ...
      for(TEntityMap::iterator itEntities = entitiesMap.begin();
          itEntities != entitiesMap.end();
          ++itEntities) {
         glPushMatrix();
         (itEntities->second)->Accept(*m_pcVisitor);
         glPopMatrix();
      }
      if(m_bAntiAliasing) {
