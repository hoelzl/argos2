...
/**
 * @file <argos2/simulator/space/space_multi_thread.cpp>
 *
...
namespace argos {
   ...
   void CleanupUpdateThread(void* p_data) {
   ...
   void* LaunchUpdateThread(void* p_data) {
      LOG.AddThreadSafeBuffer();
      LOGERR.AddThreadSafeBuffer();
      CSpaceMultiThread::SUpdateThreadData* psData = reinterpret_cast<CSpaceMultiThread::SUpdateThreadData*>(p_data);
   ...
   void CSpaceMultiThread::Init(TConfigurationNode& t_tree)
   {
   ...
   void CSpaceMultiThread::StartThreads() {
      int nErrors;
   ...
   void CSpaceMultiThread::Destroy()
   {
   ...
   void CSpaceMultiThread::AddControllableEntity(CControllableEntity& c_entity) {
   ...
   void CSpaceMultiThread::RemoveControllableEntity(CControllableEntity& c_entity) {
      m_bIsControllableEntityAssignmentRecalculationNeeded = true;
   ...
   void CSpaceMultiThread::SetPhysicsEngines(TPhysicsEngineList& t_engines) {
...
#define MAIN_WAIT_FOR_PHASE_END(PHASE)                                  \
   ...
   void CSpaceMultiThread::UpdateControllableEntities() {
   ...
   void CSpaceMultiThread::UpdatePhysics() {
...
#define THREAD_SIGNAL_PHASE_DONE(PHASE)                     \
   ...
   void CSpaceMultiThread::UpdateThread(UInt32 un_id) {
      /* Copy the id */
      UInt32 unId = un_id;
