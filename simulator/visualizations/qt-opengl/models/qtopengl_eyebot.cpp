...
namespace argos {
   ...
   void CQTOpenGLEyeBot::Draw(CEyeBotEntity& c_entity) {
      ...
      const CColor& cLowColor = cLEDEquippedEntity.GetLED(1).GetColor();
      ...
      for(UInt32 i = 1; i < 16; i++) {
         ...
         const CColor& cUpColor = cLEDEquippedEntity.GetLED(i+i).GetColor();
         const CColor& cLowColor = cLEDEquippedEntity.GetLED(i+i+1).GetColor();
         SetLEDMaterial(cUpColor.GetRed(),
                        cUpColor.GetGreen(),
                        cUpColor.GetBlue());
         glCallList(m_unSideLEDList);
